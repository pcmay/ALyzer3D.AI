...
# Expects: paper_model_scalar_pathway_v1_minus5_stripped_80_20_seed1, etc.
...
# Expects: /Users/PeterMay/Downloads/amyloidosis/combined_80_20_seed1/test, etc.
TEST_DATA_PATTERN = "/Users/PeterMay/Downloads/amyloidosis/combined_80_20_seed{}/test"

# 3. Seeds to evaluate
SEED_RANGE = range(0, 12)  # Runs seed 1 through 10. Adjust if you have fewer.
...
def prepare_test_data(data_dir, max_length, tokenizer, plm_model, device, type_mapping):
    ...
    for class_label, class_name in enumerate(["non_amyloid", "amyloid"]):
        ...
        for base_name, files in tqdm(protein_files.items(), desc=f"Processing {class_name}"):
            ...
            # --- Robust Length Check (from 80_20 script) ---
...
def evaluate_seed(seed_num, tokenizer, plm_model, device, lc_map):
    """
    Evaluates a specific 80_20 Seed.
    Loads the specific test data for that seed and the specific models.
    """
    ...
    # 3. Load & Prepare Data (Unique to this seed)
    ...
    # Combine scalar features (pI, GRAVY, Arom, MW, RoG) -> 5 dims
...
def main():
    ...
    # Iterate over Seeds instead of Versions
    for seed in SEED_RANGE:
        res = evaluate_seed(seed, tokenizer, plm_model, device, lc_map)
        if res:
    ...
    if results_summary:
        ...
        print(df.round(4))
        df.to_csv("stratified_metrics_80_20_seeds_summary.csv", index=False)
        print("\nSaved to 'stratified_metrics_80_20_seeds_summary.csv'")
