...
VERSION_RANGE = range(1, 13)  # Runs v1 through v12

MAX_LENGTH = 120
TEST_DATA_DIR = "/Users/PeterMay/Downloads/amyloidosis/rev_colab_test"
KAPPA_LAMBDA_CSV = "kappaorlambda.csv" # Path to your lookup file
...
def prepare_test_data(data_dir, max_length, tokenizer, plm_model, device, type_mapping):
    ...
    print("Preparing test data and calculating features...")
...
# --- 2. Evaluation Logic per Version ---

def calculate_subgroup_metrics(y_true, y_prob, name):
    ...
    if len(np.unique(y_true)) < 2:
        # Cannot calculate AUC if only one class exists in the subset
        auc_val = np.nan 
        pr_auc_val = np.nan
    else:
    ...
    # --- 2. Threshold-Dependent Metrics (at 0.5) ---
    y_pred_class = (y_prob > 0.5).astype("int32")
...
def evaluate_version(version_num, base_test, features_test):
    model_dir = DIR_PATTERN.format(version_num)
    print(f"\n" + "="*60)
    print(f" PROCESSING VERSION {version_num}: {model_dir}")
    ...
    results_row = {"Version": f"v{version_num}"}
    ...
    for name, mask in subsets:
        ...
        metrics = calculate_subgroup_metrics(y_sub_true, y_sub_prob, name)
    ...
    axes[0].set_title(f"v{version_num} ROC Curves")
    axes[0].set_xlabel("False Positive Rate")
    axes[0].set_ylabel("True Positive Rate")
    axes[0].legend(loc="lower right")
    axes[0].grid(alpha=0.3)

    axes[1].set_title(f"v{version_num} Precision-Recall Curves")
    ...
    plt.tight_layout()
    save_path = os.path.join(model_dir, f"evaluation_stratified_v{version_num}.png")
...
def main():
    ...
    if not os.path.exists(TEST_DATA_DIR):
        print(f"ERROR: {TEST_DATA_DIR} not found.")
        return

    base_test, features_test = prepare_test_data(TEST_DATA_DIR, MAX_LENGTH, tokenizer, plm_model, device, lc_map)
    ...
    for v in VERSION_RANGE:
        res = evaluate_version(v, base_test, features_test)
        if res:
    ...
    if results_summary:
        ...
        core_cols = ["Version"]
        # Define the order of metrics we want to see
        ...
        print("\n\n" + "="*30 + " STRATIFIED SUMMARY " + "="*30)
        print(df.round(4))
        df.to_csv("stratified_metrics_summary.csv", index=False)
        print("\nSaved to 'stratified_metrics_summary.csv'")
