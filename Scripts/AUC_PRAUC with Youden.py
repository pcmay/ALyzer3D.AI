...
OPTIMAL_COLOR = 'red' # Color for Youden point
...
def main():
    ...
    print(f"Optimal Threshold (J_max): {best_thresh:.4f}")
    print(f"At Optimal: Sensitivity={opt_sens:.4f}, Specificity={opt_spec:.4f}")

    # Calculate Precision for the optimal threshold for the PR plot
    y_pred_opt = (y_prob >= best_thresh).astype(int)
    tn_opt, fp_opt, fn_opt, tp_opt = confusion_matrix(y_true, y_pred_opt, labels=[0, 1]).ravel()
    opt_prec = tp_opt / (tp_opt + fp_opt) if (tp_opt + fp_opt) > 0 else 0.0
    ...
    # Plot Youden Optimal Threshold
    plt.scatter(opt_fpr, opt_sens, color=OPTIMAL_COLOR, s=MARKER_SIZE*1.5, marker='*', zorder=10,
                label=f'Optimal t={best_thresh:.2f} (Youden)')
    plt.annotate(f"t={best_thresh:.2f}", (opt_fpr, opt_sens), xytext=(10, -15), textcoords='offset points',
                 fontsize=14, color=OPTIMAL_COLOR, fontweight='bold')
    ...
    # Plot Youden Optimal Threshold on PR Curve
    plt.scatter(opt_sens, opt_prec, color=OPTIMAL_COLOR, s=MARKER_SIZE*1.5, marker='*', zorder=10,
                label=f'Optimal t={best_thresh:.2f} (Youden)')
    plt.annotate(f"t={best_thresh:.2f}", (opt_sens, opt_prec), xytext=(-20, 15), textcoords='offset points',
                 fontsize=14, color=OPTIMAL_COLOR, fontweight='bold')
